...
export default function Chat() {
  ...
  const resumeId       = searchParams.get('conv')
    ...
    const init = async () => {
      ...
      if (resumeId) {
        ...
        msgs.forEach(m => addMessage({
          ...
          options: m.options, diagnostico: m.diagnostico,
        }))
        setConvId(resumeId)
      } else {
  ...
  return (
    <div className="flex flex-col h-dvh bg-blanco">
      ...
      {/* ── HEADER ── */}
      <header className="bg-noche flex-shrink-0">
        {/* Espaciador barra de estado — mismo patrón que AppShell */}
        <div className="safe-top" />
        {/* Contenido del header */}
        <div className="px-4 py-3 flex items-center gap-2">
          <button
            onClick={() => navigate(-1)}
            className="text-lavanda p-1 -ml-1 flex-shrink-0"
          >
              ...
              <div className="relative">
                ...
                <AnimatePresence>
                  {showDiagList && (
                    ...
                    >
                      {diagnoses.map((d) => {
                        const levelColor = d.nivel === 'alto' ? 'bg-rosa'
                          : d.nivel === 'medio' ? 'bg-lavanda' : 'bg-menta'
                        return (
                          <button
                            key={`${d.tipo}-${d.nivel}`}
          ...
          {messages.map((msg) => {
            ...
            return (
              <div key={msg.id}>
                ...
                {msg.diagnostico && (
                  <div
                    ref={msg.id === lastBotId ? diagnosisRef : null}
      ...
      {/* ── INPUT ── */}
      <div className="bg-noche flex-shrink-0">
        ...
        {/* Espaciador barra de navegación */}
        ...
        <div className="bubble-bot px-3.5 py-2.5">
          <p
            ...
            dangerouslySetInnerHTML={{
              __html: message.content.replace(/\*\*(.*?)\*\*/g, '<strong class="text-violeta">$1</strong>')
            }}
          />
        </div>
      </div>
